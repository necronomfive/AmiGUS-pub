...
void hagenPlayNote(APTR base, UWORD chNr, UWORD midNote, UWORD midVel, UWORD ctrl)
{
	ULONG freq;
	UWORD fVel;
...
int main(int argc,char **argv)
{
    ...
    if (filehandle = Open(argv[1],MODE_OLDFILE))
    {
		...
		if (filesize != 0)
		{
			...
			if (memory = AllocMem(filesize,MEMF_ANY)){}
			else
			{
			...
			if (Read(filehandle, memory, filesize)==-1)
			{
			...
			initSample(memory,board_base,filesize>>2);
			FreeMem(memory,filesize);
			...
			for (i=0; i<32; i++)
			{
				...
				WriteReg32(board_base,HAGEN_VOICE_PENDH,(ULONG)filesize);
						...
						if (CheckIO((struct IORequest *)SerialIO) ) /* If request is complete... */
						{
							...
							else if (midiCnt != 3)
							{
								...
								if (midiCnt==2)
								{
									...
									{
										midiVel = midiDat;
										hagenPlayNote(board_base, (UWORD)playvoice, (UWORD)midiFreq, (UWORD)midiVel, control_word);
										playvoice++;
											if (playvoice == 32)
												playvoice = 0;
